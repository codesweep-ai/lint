...
//
// `make lint` runs it with `go tool -modfile=go.golangci.mod`, so a fresh
// checkout needs no install and every machine runs the version recorded here.
// It sits apart from go.mod for two reasons. Its dependency tree is some two
// hundred modules and none of them belong in what this module publishes. And
// minimal version selection resolves one version per module across a whole
// file: golangci-lint and actionlint disagree about go.yaml.in/yaml/v4, so a
// file holding both builds neither.
module github.com/codesweep-ai/lint/internal/golangci
